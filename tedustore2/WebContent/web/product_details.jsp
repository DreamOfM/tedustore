        ...
        <div id="icon_all">
            <ul id="icon_list">
            	<c:forEach items="${listPic}" var="pic" >
	                <li class="i1"><img width="84px" src="../${pic.url}"/></li>
            	</c:forEach>
            </ul>
        </div>
    </div>
    ...
    <div class="right_detail lf">
        ...
        <p class="spec">
            ...
            <c:forEach items="${listSeries}" var="series">
            	<c:if test="${series.id==goods.id}">
					<a href="showGoodsInfo.do?id=${series.id}&categoryId=${goods.categoryId}" 
							class="avenge borderChange" style="width:auto;height:auto; border: 1px solid rgb(102, 102, 102); color: rgb(102, 102, 102);">
						${series.spec}
					</a>
            	</c:if>
            	<c:if test="${series.id != goods.id}">
					<a href="showGoodsInfo.do?id=${series.id}&categoryId=${goods.categoryId}" 
							class="avenge" style="width:auto;height:auto;border: 1px solid rgb(102, 102, 102); color: rgb(102, 102, 102);">
						${series.spec}
					</a>
            	</c:if>
            </c:forEach>
        ...
        <!-- 购买部分-->
        <div class="shops" style="margin-top:auto !important">
            <a href="${pageContext.request.contextPath}/cart/showCart.do" class="buy lf" id="buy_now">立即购买</a>
        ...
        <div style="text-align: center;">
        	<span id="addCartMsg" style="display: none"><!-- class="succee" --> 
                 ...
                 <c:forEach items="${goodses}" var="goods">
	                 <div class="detail_1 lf" >
	                    <div class="detail_img1">
	                    	<a href="showGoodsInfo.do?id=${goods.id}&categoryId=${goods.categoryId}">
	                        	<img src="..${goods.image}" border="0">
	                        	 
	                        </a>
	                    </div>
	                   <p style="margin-top:auto !important">${goods.title}</p>
	                </div>
                </c:forEach>
                <!--</div>-->
            </div>
...
<script type="text/javascript">
    ...
    $("#add_cart").click(function (e) {
    	buyAccount=$("#buy-num").val();
    	goodsId=${goods.id};
    	console.log(buyAccount+goodsId);
		$.ajax({
			"url":"${pageContext.request.contextPath}/cart/addCart.do",
			"data":"goodsId="+goodsId+"&count="+buyAccount,
			"type":"GET",
			"dataType":"json",
...
/* 商品详情页  */
	function toItemInfo(id,categoryId) {
		if (id) {
			window.location="${pageContext.request.contextPath}/goods/showGoodsInfo.do?id="+id+"&categoryId="+categoryId;
		}else {
			alert("商品id不存在");
		}
	} 
...
//             window.location.href = url;
        ...
        $("#buy_now").click(function(e){
    		...
    		$.ajax({
    			...
    			"async":false //设置为同步，等待返回结果后，才进入下一步
    		});
