    ...
    <div class="adress_choice">
        ...
        <c:forEach items="${listAddress}" var="address">
	        <c:if test="${address.isDefault==1}">
	        	<div id="${address.id}" class="address base_select">
	        </c:if>
	        <c:if test="${address.isDefault!=1}">
	        	<div id="${address.id}" class="address base">
	        </c:if>
...
<script>
    $(".go_pay").click(function () {
    	...
    	var addressid = $(".base_select").attr("id");
    	var url = "../order/showPayment.do?addressid="+addressid;
    	//alert(url);
        location.href = url;
    })
</script>
...
<script>
	...
	$("div .address").click(function(){
		if($(this).hasClass("base_select")){
			alert("已选择该地址为 默认收货地址。");
			return;
		}
		$(this).attr("class","address base_select");
		$(this).siblings().attr("class","address base");
		$(this).find(".user_site").hide();
		
		var addressId = $(this).attr("id");
		
		console.log("userid:" + ${sessionScope.user.id});
		console.log("新的默认收货地址id:" +addressId);
		
	})
	/*
	$(".user_site").click(function(){
		$(this).parent().attr("class","address base_select");
		$(this).parent().siblings().attr("class","address base");
		$(this).hide();
