...
<body>
	...
	<div id="shop_detail">
		...
		<div id="preview" class="lf">
			<div id="mediumDiv">
				<img id="mImg" width="460px" height="360px" src="..${goods.image}" />
			</div>
			<div id="icon_all">
				<ul id="icon_list">
					<c:forEach items="${listPic}" var="pic">
						<li class="i1"><img width="84px1" src="../${pic.url}" /></li>
					</c:forEach>
				</ul>
			</div>
		</div>
		...
		<div class="right_detail lf">
			...
			<h1>${goods.title }</h1>
			<!-- 商品卖点 -->
			<h3>${goods.sellPoint }</h3>
			<!-- 价格部分-->
			<div class="price">
				<div id="pro_price">
					<b>学员售价：</b><span>￥${goods.price }</span>
				</div>
				<div class="promise">
        ...
        <c:forEach items="${list1F}" var="gc" varStatus="s">
	                <p>${gc.name}</p>
	                <ul>
		                <c:forEach items="${list1FSubs[s.index]}" var="goodsCategory">
		                    <li>
        ...
        -->
			<p class="style" id="choose_color">
				<s class="color">颜色：</s>
				<c:foreach items="${list2f}" var="co" varStatus="s">
					<ul>
						<c:forEach items="${list1FSubs[s.index]}" var="goodsCategory">
							<li><a
								href="${pageContext.request.contextPath}/goods/showGoods.do?categoryId=${goodsCategory.id}">
									${goodsCategory.name} </a></li>
						</c:forEach>
					</ul>
			...
			<p class="spec">
				...
				<c:forEach items="${listSeries}" var="series">
					<c:if test="${series.goodsId==goods.id}">
						<a
							href="showGoodsInfo.do?id=${series.goodsId}&categoryId=${categoryId}"
							class="avenge borderChange"
							style="border: 1px solid rgb(102, 102, 102); color: rgb(102, 102, 102);">
							${series.spec} </a>
					</c:if>
					<c:if test="${series.goodsId != goods.id}">
						<a
			...
			<!-- 购买部分-->
			<div class="shops">
				<a href="${pageContext.request.contextPath}/cart/showCart.do"
					class="buy lf" id="buy_now">立即购买</a> <a href="#" class="shop lf"
					id="add_cart"> <img
			...
			<div style="text-align: center;">
				<span id="addCartMsg" style="display: none">
					<!-- class="succee" --> <img src="../images/cart/product_true.png"
					alt="" /> <span>加入成功</span>
				</span>
			</div>
		</div>
					...
					<c:forEach items="${listGoods}" var="goods">
						<div class="detail_1 lf">
							<div class="detail_img1">
								<a
									href="showGoodsInfo.do?id=${goods.id}&categoryId=${goods.categoryId}">
	...
	<script type="text/javascript">
		...
		$("#add_cart").click(function(e) {
			...
			$.ajax({
				"url" : "../cart/addCart.do",
				"data" : "goodsid=${goods.id}&count=" + $("#buy-num").val(),
				"type" : "GET",
				"dataType" : "json",
